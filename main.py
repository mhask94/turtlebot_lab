...
__usage__ = '\nUsage:\tpython3 main.py <process_filename>.mat ' + \
            '<truth_filename>.mat [True/False->animate]'
...
if __name__ == "__main__":
    ...
    if len(args) == 0:
        ...
        mat_file = args[0]
        mat_file_truth = args[1]
        if not mat_file[-4:] == '.mat':
            __error__("Invalid file type" + __usage__)
        if not mat_file_truth[-4:] == '.mat':
            __error__("Invalid file type" + __usage__)
        if len(args) == 3:
            animate = args[2][0] == 'T' or args[2][0] == 't'
    else:
        __error__("Invalid number of arguments received" + __usage__)
